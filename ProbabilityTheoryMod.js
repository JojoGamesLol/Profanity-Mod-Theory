...
var tick = (elapsedTime, multiplier) => {
    ...
    if (rpgUnlock.level > 0) {
        if(rpgBestLemma > 50*rpgTheory) {
            rpgBestLemma = 50*rpgTheory-1;
        }
        ...
        if (rpgLemma > 0) {
            ...
            theirHealth -= Math.min(1, (dicePoints2 * ourHealth * dt) / theirMaxHealth * atkMult);
...
var getSecondaryEquation = () => {
    ...
    if (stage == 1) rVal = `\\dot{\\rho_2} = ${perm1.level > 0 ? "(r_5 + 1)" : ""} \\sum^{8}_{i=5}{r_i}, \\quad \\dot{r_i} = \\frac{-r_i}{${getSkillEffect(9)}}`;
        ...
        let multi = Math.min(gacha, (1 + gachaBulk.level)*extra_mult);
            ...
            children: [
                ui.createGrid({
                    ...
                    children: [
                        ...
                        ui.createLatexLabel({ 
                            text: () => (dicePoints * ourHealth) + " / " + dicePoints, 
                            fontSize: 10,
                        ...
                        ui.createLatexLabel({ 
                            text: () => (theirMaxHealth * theirHealth) + " / " + theirMaxHealth, 
                            fontSize: 10, 
        ...
        if (stage == 2) {
            ...
            for (let n = 0; n < multi; n++) {
                ...
                for (let a = 0; a < 6; a++) {
                    ...
                    stars[a] += rand * (gachaValue2.level + 1)*extra_mult;
                    if (rand > 0) break;
                    osum -= odds[a];
                }
                tSkill = 0;
    ...
    for (let a = 0; a < 6; a++) {
        btns[a] = ui.createButton({
            ...
            onClicked: () => {
                ...
                stars[a] -= max;
                btns[a].opacity = stars[a] >= 1 ? 1 : 0.5;
                btns[a].inputTransparent = stars[a] >= 1 ? false : true;
            ...
            children: [
                ...
                ui.createButton({
                    ...
                    onClicked: () => {
                        let upg = [d4Master, d6Master, d8Master, d10Master, d12Master, d20Master][id];
                        while (upg.level > 0) {
                            let c = cost.getCost(upg.level-1);
                            currency3.value += c;
                            upg.level--;
                        }
...
var showBoosterPopup = () => {
    ...
    let updateButtons = () => {
        ...
        for (let a = 0; a < 4; a++) {
            ...
            pBoosterButtons[a].opacity = sum < pBoostersBought ? 1 : 0.5;
            pBoosterButtons[a].inputTransparent = sum < pBoostersBought ? false : true;
        }
    }
    updateButtons();
